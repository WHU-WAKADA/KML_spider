...
for i in range(751, 1000):
    ...
    service = Service(executable_path='C:/ProgramData/anaconda3/envs/game/chromedriver.exe')
    ...
    for i in href:
        driver.get(i)
        time.sleep(1)
        error_location = pyautogui.locateOnScreen('../resource/error.png', confidence=0.95)
        if error_location is not None:
            ga.clickpng('../resource/error.png', 0.95, 0.5)
        save_location = pyautogui.locateOnScreen('../resource/save.png',confidence=0.95)
        if save_location is not None:
            ga.clickpng('../resource/save.png', 0.95, 0.5)
        time.sleep(1)
